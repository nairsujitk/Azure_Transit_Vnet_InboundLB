...
data "www_build1" "www_script1" {

  template = "${file("${path.root}${var.www_script_path1}")}"
}
data "www_script_config1" "config1" {
  gzip          = true
  base64_encode = true

  part {
    content = "${data.www_build1.www_script1.rendered}"
  }
}
...
resource "azurerm_virtual_machine" "app1-host1" {
  ...
  os_profile {
    ...
    custom_data             = "${data.www_build1.www_script1.rendered}"
  }
...
data "www_build2" "www_script2" {

  template = "${file("${path.root}${var.www_script_path2}")}"
}
data "www_script_config2" "config2" {
  gzip          = true
  base64_encode = true

  part {
    content = "${data.www_build2.www_script2.rendered}"
  }
}
...
resource "azurerm_virtual_machine" "app2-host1" {
  ...
  os_profile {
    ...
    admin_username          = "${var.adminUsername}"
    admin_password          = "${var.adminPassword}"
    custom_data             = "${data.www_build2.www_script2.rendered}"
  }
